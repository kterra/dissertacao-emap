...
def distance_matrix_request_builder(origins, origin_start, destinations,mode = MODE_TRANSIT, transit_mode = BUS, traffic_model = BEST_GUESS, departure_time = "now" ):
    ...
    if not os.path.exists(os.path.join("output", 'distance_matrix_distance_result.csv')):
    ...
    if not os.path.exists(os.path.join("output", 'distance_matrix_duration_result.csv')):
    ...
    for origin in origins[origin_start:]:
        ...
        try:
            ...
            with open(os.path.join("output", 'log_distance_matrix.csv'), 'a') as log_file:
                log_file.write("{},\"{}\"\n".format(origins.index(origin),str(origin[0] + "," + re.sub("\n","", origin[1]))))
            break
